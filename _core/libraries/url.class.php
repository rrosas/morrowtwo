...
class Url
	...
	public function makeUrl($path, $query=array(), $replacements=array(), $rel2abs=false, $sep=null)
		...
		if(empty($scheme))
			...
			// path: trim slashes
			if($path != '') $path = HelperUrl::trimSlashes($path);
			...
			else if($this->language_get != $this->language_default) #$this->config_all['language']['default'])
				{
			...
			// otherwise add lang and project path and slash
				...
				// handle modrewrite
				if(!$this->config_all['url']['modrewrite'])
					{
					$query['morrow_content'] = $path;
					$path = '';
					}
				}
			...
			if ($rel2abs)
				...
				$domain_parts['path'] = HelperUrl::trimSlashes($domain_parts['path']);
