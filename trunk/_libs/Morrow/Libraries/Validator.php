...
class Validator {
	public static function checkUsername($value, &$error) {
		...
		if(!preg_match("=^[a-z0-9_]{5,20}$=i", $value)) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Invalid Username. Allowed are only a-z, 0-9, _. Additionally the length must be between 5 and 20 characters.');
			return false;
		}
		return true;
	}
	...
	public static function checkJpegFile($value, &$error) {
		...
		$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Image is not a valid JPEG.');
		return false;
	}
	...
	public static function checkEmail($var, &$error) {
		...
		if (!preg_match("=^$localpart+(\.$localpart+)*@($domainpart+\.$domainpart+)$=i", $var, $match)) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Email address is not valid.');
			return false;
		}
	...
	public static function checkPassword($value, &$error, $compare_value=null) {
		if(strlen($value) > 0 && strlen($value) < 5) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Passwords must be at least 5 characters long.');
			return false;
		}
		if($compare_value !== null && $value != $compare_value) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Passwords are not identical.');
			return false;
		}
		return true;
	}
	...
	public static function checkCurrency($value, &$error, $compare_value=null, $locale) {
		...
		if(!preg_match("=^[0-9]{1,}({$sep}[0-9]{0,4})?$=",$value)) { 
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Price not valid.');
			return false;
		}
		return true;
	}
	...
	public static function checkValidDate($value, &$error) {
		if(!preg_match('/([0-9]{0,4})-([0-9]{0,2})-([0-9]{0,2})/',$value, $matches)) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('The date is not valid.');
			return false;
		}
		$y = $matches[1];
		$m = $matches[2];
	...
	public static function check18($birthday, &$error) {
		...
		return \Morrow\Core\Factory::load('Libraries\Language')->_('You must be at least 18 years old.');
	}
	
	public static function checkGermanZip($value, &$error) {
		 if(!preg_match("=^[0-9]{5,5}*$=",$value)) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Zip is not valid.');
			return false;
		 }
		return true;
	}
	...
	public static function checkCaptcha($captcha, &$error) {
		$session = \Morrow\Core\Factory::load('session');
		$session_captcha = $session->get('captcha');
		$session->delete('captcha');
		if (strtolower($captcha) != strtolower($session_captcha)) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Captcha does not match.');
			return false;
		}
		return true;
	}
	...
	public static function checkInteger($value, &$error) {
		if(!preg_match('=^[0-9]*$=',$value)) { 
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('Not a number.');
			return false;
		}
		return true;
	}
	...
	public static function checkEAN($value, &$error) {
		...
		if($return != true) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('EAN Code not valid.');
		}
	...
	public static function checkUPC($value, &$error) {
		...
		if($return != true) {
			$error = \Morrow\Core\Factory::load('Libraries\Language')->_('UPC Code not valid.');
		}
